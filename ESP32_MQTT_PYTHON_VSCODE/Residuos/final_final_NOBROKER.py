...
def limpiar_eventos():
    global eventos_temporales, eventos_enviados
    ...
    # Eliminar eventos de eventos temporales una vez enviados al broker MQTT
    eventos_temporales.difference_update(eventos_enviados)
...
def registrar_evento(tipo_evento):
    ...
    eventos_temporales.add(ujson.dumps(evento))  # Agregar a eventos temporales
...
def enviar_evento_al_broker(client, evento):
    result = client.publish(b"topic/eventos", evento, qos=1)  # Enviar evento al broker MQTT con QoS 1
    if result:
        eventos_enviados.add(evento)  # Agregar evento a eventos enviados
...
def main():  
    ...
    while True:
        ...
        for evento in list(eventos_temporales):
            if enviar_evento_al_broker(client, evento):
                eventos_temporales.remove(evento)
